...
public class FileStorageManager {
	...
	// Réprtoire racine de notre stockage
	...
	// Cette fonction servira à stocker le fichier à la demande d'un DAO (ici repositories)
	public boolean saveFile(String entityName, int id, File f) {
		if (storageRoot == null
				|| !storageRoot.exists()
				|| !storageRoot.isDirectory()) {
			// Problème, pas de repertoire de base correct
			log.error("repertoire de base de stockage non disponnible");
			return false;
		}
		// Génération du nom du répertoire pour notre entité
		...
		// Si le répertoire pour mon dao/entite n'eciste pas, je le cree
		if ( !entityRep.exists() )
			entityRep.mkdirs();
	...
	public Optional<File> getFile(String entityName, int id) {
		if (storageRoot == null
				|| !storageRoot.exists()
				|| !storageRoot.isDirectory()) {
			// cas d'erreur envisagé: pas de répertoire de base correct
			log.error("repertoire de base de stockage non disponnible");
		...
		// si le repertoire n'existe pas, pas de fichier à renvoyer
